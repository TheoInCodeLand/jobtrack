#!/usr/bin/env python3
# api/ats_analyzer.py - Vercel serverless ATS analysis service
...
# Try to import PDF extraction libraries (may not be available on Vercel due to size)
try:
    import pdfplumber
    PDF_SUPPORT = True
except ImportError:
...
class ATSAnalyzer:
    ...
    def extract_text_from_pdf(self, pdf_path: str) -> str:
        if not PDF_SUPPORT:
            raise ImportError("pdfplumber not installed. PDF extraction disabled on serverless.")
    ...
    def extract_text_from_docx(self, docx_path: str) -> str:
        if not DOCX_SUPPORT:
            raise ImportError("python-docx not installed. DOCX extraction disabled on serverless.")
...
def handler(request, context):
    """
    Vercel serverless function handler for ATS analysis
    Expects: POST request with JSON body containing cv_text and job_description
    """
    try:
        ...
        if not cv_text or not job_description:
            return {
                "statusCode": 400,
                "body": json.dumps({
                    "error": "Missing required fields: cv_text and job_description"
                })
            }
        ...
        # For serverless, we analyze text directly rather than file paths
        # The Node.js layer should extract text and send it
        jd_text = analyzer.clean_text(job_description)
        cv_clean = analyzer.clean_text(cv_text)
        
        semantic_score = analyzer.calculate_semantic_similarity(cv_clean, jd_text)
        matching_skills = analyzer.find_matching_strengths(cv_clean, jd_text)
        missing_skills = analyzer.find_missing_critical_skills(cv_clean, jd_text)
        experience_analysis = analyzer.calculate_experience_match(cv_clean, jd_text)
        formatting_issues = analyzer.check_formatting_issues(cv_clean)
        ...
        feedback = analyzer.generate_brutal_feedback(
            final_score, matching_skills, missing_skills, 
            experience_analysis, formatting_issues
        )
