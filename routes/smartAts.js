...
/**
 * Unified AI analysis function - prioritizes GROQ over Gemini
 */
async function analyzeWithAI(params) {
    const { resumeText, jobDescription, jobLevel, jobType, applicationData } = params;
    
    // PRIORITY: Use Groq if available, otherwise Gemini, otherwise error
    const useGroq = !!GROQ_API_KEY;
    const useGemini = !useGroq && !!GEMINI_API_KEY;
    
    if (!useGroq && !useGemini) {
        throw new Error('No AI API configured. Set GROQ_API_KEY (preferred) or GEMINI_API_KEY environment variable.');
    }
    ...
    try {
        if (useGroq) {
            ...
            apiName = 'Gemini';
            console.log('[AI] Using Gemini API (fallback)');
...
/**
 * Call Gemini API (Google format) - Fallback only
 */
...
router.post('/analyze', async (req, res) => {
    ...
    try {
        ...
        // Analyze with AI (Groq preferred, Gemini fallback)
